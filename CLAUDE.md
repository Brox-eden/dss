...
- Domain is `dss-sa.com` (see `public/CNAME`). The contact email is `info@dss-sa.com`, and the Web3Forms access key in `components/ContactForm.tsx` was regenerated against that address on 2026-09-16 — both the displayed email and the actual form-delivery destination are now consistent.
